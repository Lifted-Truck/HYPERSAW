...
*Last verified current: 2026-07-19.* (ROADMAP.md is the authoritative status trail; this is the human-readable snapshot.)
...
- **Phases 0–3 CLOSED.** The plugin is a shippable, playable instrument with **two selectable engines** (SAW / SPECTRA, engine selector param) and the dynamics layer live inside SAW:
  - **SAW**: full parameter surface — all four detune laws (incl. host-tempo-synced tempo-grid), seeded distributions, drift, ADSR (ADR-021), density comp, width + super-width, mono/glide/legato, digital↔clean, phase scatter, pan scatter.
...
- **Track E (effects line), in progress** on the shared **force core** (ADR-034, E0): the position-domain home/sync/splay/gravity/drift/inertia system extracted as one module all four effect engines consume, verified seed-for-seed by `force_check`. **E1 increment 1 shipped**: the resonator bank (`filter_core.h`) is bit-exact (RMS 0.0, 11/11) against `swarmfilter.html` with L0-14 collapse→Q, via `filter_check`. Remaining E1: notch swarm (L0-16/17), external-audio input, effect-plugin shell target.
- **The oracle** (`./verify fast|full`): leak/structure gates → Release build → golden self-check → six oracle chains: L0-1 parity (dual-engine goldens regenerated from the HTML references every run), trajectory suite (L0-2..5, 8..13, 22 + ADR-015 anchors), state persistence, **force-core** trajectories (L0-14/15/19/20 population halves), **spectra** parity + L0-6/7 + the P=1 measured-equivalence gate (ΔR=0), and **filter** audio parity + L0-14. CI mirrors it on every push (macOS + Windows build + pluginval strictness 5).
