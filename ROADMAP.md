...
- **Mod matrix design (human request, 2026-07-18) — DESIGN SESSION wanted before build.** The human has ideas and wants to work through plans first. Sources beyond the traditional (envelopes, LFOs): a **Kuramoto LFO** — a swarm of phase oscillators where every parameter routed to it becomes a member of one coupled population, so connected parameters sync/desync as a group under a pull-K (the instrument's own physics turned into a modulation source; K<0 splays them into deliberate phase-offset spread, K>0 locks them into unison motion). This is the signature mod source, distinct from N independent LFOs. Design questions for the session: per-destination depth/polarity vs per-member coupling; whether the Kuramoto LFO reuses force_core (position-domain) or a dedicated phase population; how R/σ of the modulator itself become meta-sources. Matrix scaffolding (sources × destinations, depth, curve) is standard; the Kuramoto LFO is the novel core. Reduce-first: destinations are the existing frozen param ids; a source only needs a per-block scalar.
